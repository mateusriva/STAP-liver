...
def build_graph(volume,labelmap,add_edges=True, target_vertices=None):
    ...
    if target_vertices is None:
        ...
        vertices = np.empty((target_vertices, 4))
        ...
        for i, label in enumerate(range(target_vertices)):
            if label in actual_labels:
                ...
                vertices[label] = np.array([math.inf]*4)

    if not add_edges:
        return SRG(vertices, np.array([]), ["centroid_x", "centroid_y", "intensity", "size"] ,[])
    else:
        # Compute relational attributes
        centroids = vertices[:,:2]
        intensities = vertices[:,2]
        sizes = vertices[:,3]
        ...
        return SRG(vertices, edges, ["centroid_x", "centroid_y", "intensity", "size"], ["position_x","position_y","contrast","ratio"])#, "contrast", "ratio"])
