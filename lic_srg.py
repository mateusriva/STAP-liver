...
class Matching:
    ...
    def vertex_cost(self, weights=None):
        """Computes the vertex cost of this solution.
        ...
        -------
        cost : `float`
            Vertex cost of the solution, weighted.
        """
        if weights is None:
            weights = (1,1)
        return sum(self.observation_graph.vertexes[key].cost_to(self.model_graph.vertexes[value], weights) for key, value in self.match_dict.items())

    def edge_cost(self, weights=None):
        """Computes the edge cost of this solution.
        ...
        -------
        cost : `float`
            Edge cost of the solution, weighted.
        """
        if weights is None:
            weights = (1,1)
        return sum(
