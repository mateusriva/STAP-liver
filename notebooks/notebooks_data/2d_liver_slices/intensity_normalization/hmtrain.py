...
def histotrain(img_dir, output_dir, write_to_disk=True):
    """
    ...
    References:
        ...
        [2] M. Shah, Y. Xiao, N. Subbanna, S. Francis, D. L. Arnold,
            ...
            Med. Image Anal., vol. 15, no. 2, pp. 267–282, 2011.
    """
    input_files = io.glob_dcm(img_dir)
    out_fns = []
    ...
    joblib.dump((standard_scale, percs), 'normalization.pkl')
...
def train(img_fns, mask_fns=None, i_min=1, i_max=99, i_s_min=1, i_s_max=100, l_percentile=10, u_percentile=90, step=10):
    ...
    for i, (img_fn, mask_fn) in enumerate(zip(img_fns, mask_fns)):
        img_data = io.open_dcm(img_fn).pixel_array
        mask = None
...
def do_hist_norm(img, landmark_percs, standard_scale, mask=None):
    ...
    normed = np.zeros(img_data.shape, dtype="uint16")
    normed[mask_data > 0] = f(masked)
    img.PixelData = normed.tostring()
    return img

#if __name__ == '__main__':
#    t0 = time()
#    main('./DATASETS/NORMALIZED-FINAL/msp','./DATASETS/NORMALIZED-FINAL/NIfTI')
