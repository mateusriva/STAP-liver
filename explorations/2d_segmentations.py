...
of different 2D supersegmentation algorithms in the context of real MRI.
"""
...
def compact_watershed(observation_volume_data):
    """Sluce-by-slice compact watershed with 400 markers.
    """
    observed_labelmap_data = np.empty_like(observation_volume_data)
    # Iterating over slice
    for i in range(observation_volume_data.shape[2]):
        slice = observation_volume_data[:,:,i]
        # Applying gradient
        ...
        observed_labelmap_data[:,:,i] = watershed(magnitude, markers=400, compactness=0.001)-1
...
def h_minima_watershed(observation_volume_data, h):
    ...
    for i in range(observation_volume_data.shape[2]):
        ...
        observed_labelmap_data[:,:,i] = watershed(magnitude,markers=markers)-1
    return observed_labelmap_data
...
def local_minima_watershed(observation_volume_data):
    """Slice-by-slice watershed with all gradient local minima.
    """
    observed_labelmap_data = np.empty_like(observation_volume_data)
    # Iterating over slice
    for i in range(observation_volume_data.shape[2]):
...
def slic(observation_volume_data):
    """SLIC superpixel with 400 regions"""
    observed_labelmap_data = np.empty_like(observation_volume_data)
    # Iterating over slice
    for i in range(observation_volume_data.shape[2]):
        slice = observation_volume_data[:,:,i]
        observed_labelmap_data[:,:,i] = skis.slic(slice, n_segments=400,
                            compactness=0.0001, multichannel=False, sigma=3)
    return observed_labelmap_data

supersegmentation_functions = [compact_watershed, h_01_minima_watershed,h_001_minima_watershed,h_02_minima_watershed, slic, local_minima_watershed]
